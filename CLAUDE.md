...
OpenClaw Enchante는 OpenClaw AI 에이전트를 원클릭으로 설치하는 **Electron 데스크톱 인스톨러**. electron-vite + React + Tailwind CSS 4 기반이며 macOS/Windows를 지원한다.
...
| `openclaw-release.ts` | 고정(pin)된 OpenClaw CLI 버전 상수                          |
| `env-checker.ts`    | Node.js/OpenClaw/WSL 설치 여부 및 버전 감지 (pin 버전 기준)   |
| `installer.ts`      | Node.js, WSL, OpenClaw 자동 설치 (플랫폼별 분기)              |
| `app-settings.ts`   | Electron userData `settings.json` (Nemo shield 등)         |
| `onboarder.ts`      | OpenClaw 설정 작성, 텔레그램/잘로/라크, 스킬, 보안 상태       |
| `smoke-tests.ts`    | 설치 후 점검 (CLI, 포트, 채널 probe 등)                      |
...
| `troubleshooter.ts` | 포트 점유 확인, OpenClaw repair(`… --fix`) 실행 등 진단 도구     |
...
`useWizard` 훅이 스텝 네비게이션을 관리. 순서 (`src/renderer/src/hooks/useWizard.ts`):

`welcome` → `envCheck` → (`wslSetup`) → (`install`) → `apiKeyGuide` → `appchatGuide` → `config` → `hooks` → `done`
...
- `appchatGuide`: Telegram / Zalo / Lark 선택 (`AppchatGuideStep.tsx`)
- `hooks`: NemoClaw Shield 등 (`HooksStep.tsx`) → 이후 `done`
- `goTo()`로 스텝 건너뛰기 가능, `history` ref로 뒤로가기 지원
- 각 Step 컴포넌트는 `src/renderer/src/steps/`에 위치
- Provider 목록: `src/renderer/src/constants/providers.ts` (anthropic, google, openai, minimax, glm, deepseek, ollama 등)
...
- `docs/`: 최소 정적 페이지 (`index.html`, `privacy.html`) + **구조 문서** `APP-ARCHITECTURE.md`는 저장소 문서로만 사용 가능
- `api/newsletter.js`, `api/waitlist.js`: 서버리스 (CORS는 `enchante.cloud` 등 허용 목록)
