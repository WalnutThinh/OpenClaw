...
import OllamaPreflightPanel from './OllamaPreflightPanel'
...
interface Props {
  ...
  currentModel?: string
  /** Fallback OS when preflight has not loaded (e.g. env check error). */
...
export default function ProviderSwitchModal({
  ...
  const initProvider = (currentProvider as Provider) || 'ollama'
  ...
  const [ollamaPreflightLoading, setOllamaPreflightLoading] = useState(false)
  ...
  useEffect(() => {
    if (!isOllama || phase !== 'form') {
      setOllamaSys(null)
      setOllamaPreflightLoading(false)
      setOllamaPreflightFailed(false)
      return
    }
    let cancelled = false
    setOllamaPreflightLoading(true)
    setOllamaPreflightFailed(false)
    void window.electronAPI.env
      ...
      .finally(() => {
        if (!cancelled) setOllamaPreflightLoading(false)
      })
    return () => {
      cancelled = true
    }
