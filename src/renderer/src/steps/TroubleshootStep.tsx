...
export default function TroubleshootStep({ onBack }: TroubleshootStepProps): React.JSX.Element {
  ...
  const [tsLogTab, setTsLogTab] = useState<LogTabId>('openclaw')

  const { openclaw: tsOpenclawLogs, channels: tsChannelLogs } = useMemo(
    () => splitLogLines(logs),
    [logs]
  )
  ...
  const fixPort = async (): Promise<void> => {
    ...
    clearLogs()
    setShowLogs(true)
    await window.electronAPI.troubleshoot.fixerFix()
    setPortFixing(false)
    diagnose()
  }
  ...
  const items: DiagItem[] = [
    ...
    },
    {
      ...
      status: portStatus,
      fixLabel: t('troubleshoot.fixerFix'),
      onFix: fixPort,
      fixing: portFixing
    }
  ]
    ...
    <div className="flex-1 flex flex-col items-center justify-center px-10 gap-5">
      ...
      {logs.length > 0 && (
        <div className="w-full max-w-md">
          ...
          {showLogs && (
            <LogViewerTabs
              openclawLines={tsOpenclawLogs}
              channelLines={tsChannelLogs}
              activeTab={tsLogTab}
              onTabChange={setTsLogTab}
            />
          )}
        </div>
      )}
