...
## OpenClaw / coding agents — you *can* send email

If `~/.config/imap-smtp-email/.env` exists with valid **SMTP_** variables, **do not claim you cannot send email from OpenClaw.** Use this skill’s Node CLI from the **installed skill directory** (workspace path is usually `~/.openclaw/workspace/skills/` with a folder name like `Office-Task--Email`; legacy installs may use `Office Task--Email`).

```bash
cd ~/.openclaw/workspace/skills/Office-Task--Email   # adjust if your slug differs (see AGENTS.md inventory)
node scripts/smtp.js send --to recipient@example.com --subject "Title" --body "Plain text body"
node scripts/smtp.js send --to recipient@example.com --subject "CSV attached" --body "See attachment." --attach /root/.openclaw/workspace/gold_dollar_tracker.csv
```

- **Attachments** require the file path to fall under **`ALLOWED_READ_DIRS`** in `.env` (the Enchante/OpenClaw wizard preset includes `~/.openclaw/workspace` and `~/.openclaw/workspace/skills`).
- Prefer **running the command** over asking the user to copy-paste file contents when they asked for email delivery.
- Do **not** repeat the user’s password in chat; the CLI reads credentials from the config file only.
