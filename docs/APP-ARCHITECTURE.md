...
- **Windows:** `dist/installer/OPENCLAW-setup.exe` (bootstrapper) + `dist/EClaw-*-win.zip` hosted at `install-manifest.json` `appZipUrl`; setup downloads zip then extracts locally. Distribution rules for agents: **[AGENTS-WINDOWS-DISTRIBUTION.md](AGENTS-WINDOWS-DISTRIBUTION.md)**.
