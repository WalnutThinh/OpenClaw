...
2. **`EClaw.exe` (hoặc bản cũ `OpenClaw.exe`) vẫn đang chạy** — **tắt hết** trong Task Manager, đóng Explorer đang mở sẵn thư mục cài, tắt **Preview pane** nếu đang xem file trong thư mục đó.
...
**Kết luận:** Dùng **`EClaw-…-win.zip`** (hoặc zip cũ **`OpenClaw-…`**) **+ giải nén** là cách **ổn định** khi máy hạn chế ổ C hoặc NSIS hay lỗi; bản `.exe` tiện cho shortcut và “Apps & features”.
...
`EClaw-<version>-win.zip` (hoặc bản cũ `OpenClaw-…`) — tên chính xác xem trong `dist/`.

1. Copy file **.zip** sang **D:** (hoặc để sẵn trên D:).
2. Chuột phải → **Extract All…** / hoặc dùng **7-Zip** → giải nén **thẳng vào** một thư mục trên **D:**, ví dụ `D:\EClaw\`.
3. Chạy **`EClaw.exe`** (hoặc `OpenClaw.exe` nếu là bản zip cũ) trong thư mục vừa giải nén.

**Lưu ý:**

- Không có **shortcut tự tạo** / **Gỡ cài trong Settings** như bản NSIS; muốn shortcut thì tự tạo tới `D:\...\EClaw.exe`.
